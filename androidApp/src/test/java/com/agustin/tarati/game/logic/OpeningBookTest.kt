...
class OpeningBookTest {
    ...
    fun `the book is compiled and non-empty`() {
        assertTrue("OPENING_BOOK_ENTRIES should be populated", OpeningBook.size > 100)
    }
    ...
    fun `the initial position resolves to a legal booked move`() {
        ...
        assertNotNull("The initial position must be in the book", move)
        assertTrue("The book move must be legal", move in initial.allMovesForTurn())
    }
    ...
    fun `mirror positions yield mirror-image book moves`() {
        ...
        assertNotNull("Black's reply after 1.C1-C12 should be booked", reply)
        assertNotNull("Black's reply after the mirror of 1.C1-C12 should be booked", mirrorReply)
        assertEquals(
            "Mirror positions must recommend mirror-image moves",
            (reply ?: return).mirrored(),
            mirrorReply,
        )

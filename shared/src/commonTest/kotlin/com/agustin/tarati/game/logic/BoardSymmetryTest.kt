...
class BoardSymmetryTest {
    ...
    fun `mirror is an involution over every vertex`() {
        for (v in vertices) {
            assertEquals(v, BoardSymmetry.mirror(BoardSymmetry.mirror(v)), "mirror(mirror($v)) must be $v")
        }
    }
    ...
    fun `mirror is a bijection over the board`() {
        ...
        assertEquals(vertices.toSet(), images, "mirror must map the 23 vertices onto themselves")
    }
    ...
    fun `a move and its mirror canonicalize to the same representative`() {
        ...
        assertNotEquals(move, mirrored, "C2-C3 and its mirror must be distinct moves")

        val a = initial.canonicalMove(move)
        val b = initial.canonicalMove(mirrored)
        assertEquals(a.first, b.first, "Mirror moves must share the canonical hash")
        assertEquals(a.second, b.second, "Mirror moves must fold to the same canonical move")
    }

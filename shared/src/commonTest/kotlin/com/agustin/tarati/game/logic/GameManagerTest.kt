...
class GameManagerTest {
    ...
    fun `updateHistory with standard moves rebuilds history from initialGameState`() {
        ...
        assertEquals(2, manager.history.value.size, "History size matches move count")
        assertEquals(1, manager.moveIndex.value, "moveIndex points to last entry")
    }
    ...
    fun `updateHistory each entry holds the state AFTER its move`() {
        ...
        assertEquals(WHITE, entryState.cobs[B1]?.color, "Entry 0 state has B1 = WHITE (state after move)")
        assertNull(entryState.cobs[C1], "Entry 0 state has C1 empty (piece moved)")
    }
    ...
    fun `updateHistory with empty list clears history`() {
        ...
        assertEquals(-1, manager.moveIndex.value, "moveIndex is -1 after empty history")
    }
    ...
    fun `updateHistory entries chain correctly - each state builds on the previous`() {
        ...
        assertEquals(WHITE, stateAfterMove0.cobs[B1]?.color, "After move 0: B1 = WHITE")
        assertEquals(BLACK, stateAfterMove0.cobs[C7]?.color, "After move 0: C7 still = BLACK")

        // After move 1: C7→B4 applied on top of move 0
        assertEquals(WHITE, stateAfterMove1.cobs[B1]?.color, "After move 1: B1 still = WHITE")
        assertEquals(BLACK, stateAfterMove1.cobs[B4]?.color, "After move 1: B4 = BLACK")
        assertNull(stateAfterMove1.cobs[C7], "After move 1: C7 empty")
    }
    ...
    fun `updateHistory with custom initialState uses it as base`() {
        ...
        val entryState = manager.history.value.getGameState(0)
        assertEquals(WHITE, entryState.cobs[B1]?.color, "B1 = WHITE — moved from edited start")
        assertNull(entryState.cobs[C3], "C3 empty after move")
        // Standard opening pieces must NOT appear
        assertNull(entryState.cobs[C1], "C1 empty — no standard opening pieces")
    }
    ...
    fun `updateHistory custom initialState chains entries correctly`() {
        ...
        // After move 0: C3→B1
        assertEquals(WHITE, stateAfterMove0.cobs[B1]?.color, "After move 0: B1 = WHITE")
        assertEquals(BLACK, stateAfterMove0.cobs[C9]?.color, "After move 0: C9 still = BLACK")

        // After move 1: C9→B4
        assertEquals(WHITE, stateAfterMove1.cobs[B1]?.color, "After move 1: B1 still = WHITE")
        assertEquals(BLACK, stateAfterMove1.cobs[B4]?.color, "After move 1: B4 = BLACK")
        assertNull(stateAfterMove1.cobs[C9], "After move 1: C9 empty")
    }
    ...
    fun `updateHistory default and custom initialState produce different entry states`() {
        ...
        assertEquals(WHITE, standardEntry.cobs[B1]?.color, "Standard entry: B1 = WHITE")
        assertEquals(8, standardEntry.cobs.size, "Standard entry: 8 pieces")
    ...
    fun `undoMove past the first move restores the custom initial state`() {
        ...
        assertEquals(-1, manager.moveIndex.value, "moveIndex is -1 after undoing the only move")
        assertEquals(
    ...
    fun `clearHistory adopts the given state as the game's initial position`() {
        ...
        assertEquals(editedState, manager.initialGameState, "initialGameState tracks the cleared-to state")
        assertEquals(editedState, manager.gameState.value, "Board shows the given state")
        assertEquals(-1, manager.moveIndex.value, "moveIndex is -1")
    }
    ...
    fun `setInitialGameState stores the given state`() {
        ...
        assertEquals(editedState, manager.initialGameState, "initialGameState reflects the custom state")
    }
